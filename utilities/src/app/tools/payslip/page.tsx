...
const formatPdfMoney = (value: number) =>
  `GHS\u00A0${roundMoney(value).toLocaleString("en-US", {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2
  })}`;
...
export default function PayslipPage() {
  ...
  const calculations = useMemo(() => {
    ...
    const transportAuto = form.includeTransportAllowance ? basic * (transportRate / 100) : 0;
    const utilityAuto = form.includeUtilityAllowance ? basic * (utilityRate / 100) : 0;
    ...
    const leaveAllowanceAuto =
      form.includeLeaveAllowance ? basic * (leaveAllowanceRate / 100) : 0;
    const clothingAllowanceAuto =
      form.includeClothingAllowance ? basic * (clothingRate / 100) : 0;
    ...
    const taxableIncome = Math.max(0, grossEarnings - socialSecurity - providentDeduction165);
  ...
  const updateField = <K extends keyof PayslipForm>(field: K, value: PayslipForm[K]) => {
  ...
  const updateAllowanceAmount = (
    field: "transport" | "utility" | "rent" | "risk" | "leaveAllowance" | "clothingAllowance",
  ...
  const toFormFromEntry = (entry: PayslipEntry): PayslipForm => ({
    ...
    transportRate: toInputValue(entry.transportRate ?? DEFAULT_TRANSPORT_RATE_PERCENT),
    transport: toInputValue(entry.transport),
    includeUtilityAllowance: entry.includeUtilityAllowance ?? true,
    utilityRate: toInputValue(entry.utilityRate ?? DEFAULT_UTILITY_RATE_PERCENT),
    ...
    leaveAllowanceRate: toInputValue(entry.leaveAllowanceRate ?? DEFAULT_LEAVE_RATE_PERCENT),
    leaveAllowance: toInputValue(entry.leaveAllowance ?? 0),
    includeClothingAllowance: Boolean(entry.includeClothingAllowance),
    clothingRate: toInputValue(entry.clothingRate ?? DEFAULT_CLOTHING_RATE_PERCENT),
  ...
  const handleSaveEmployee = () => {
    ...
    if (editingEntryId) {
      setEntries((prev) =>
        prev.map((current) =>
          current.id === editingEntryId ? { ...entry, id: current.id } : current
        )
      );
      clearEditor();
      return;
  ...
  const sanitizeExportRows = (rows: Array<Array<string | number>>) =>
    rows.map((row) =>
      row.map((cell) => (typeof cell === "number" && isZeroAmount(cell) ? "" : cell))
    );

  const sanitizeJsonExport = (value: unknown): unknown => {
    if (Array.isArray(value)) return value.map((item) => sanitizeJsonExport(item));
  ...
  const exportPdf = async (
    ...
    try {
      ...
      if (isPdfChunkLoadError(error)) {
        alert("PDF module load timed out. Refresh the page and try Print Preview again.");
      } else {
    ...
    autoTable(doc, {
      startY: 80,
      head: [headers],
      body: rows.map((row) => row.map((cell) => (typeof cell === "number" ? formatMoney(cell) : String(cell)))),
  ...
  const exportStyledPayslipPdf = async (
    ...
    sourceEntries.forEach((entry, index) => {
      ...
      doc.text(`GENERATED: ${generatedAt}`, pageWidth - margin, 74, { align: "right" });
      doc.setDrawColor(90, 90, 90);
      doc.setLineWidth(0.7);
      doc.line(margin, 84, pageWidth - margin, 84);

      const earningsRows: Array<[string, number]> = [["BASIC SALARY", entry.basic]];
      if (entry.includeTransportAllowance) {
        earningsRows.push([`TRANSPORT ALLOWANCE (${formatPercent(entry.transportRate)})`, entry.transport]);
      }
      if (entry.includeUtilityAllowance) {
        earningsRows.push([`UTILITY ALLOWANCE (${formatPercent(entry.utilityRate)})`, entry.utility]);
      }
      if (entry.includeRentAllowance) {
        earningsRows.push([`RENT ALLOWANCE (${formatPercent(entry.rentRate)})`, entry.rent]);
      }
      if (entry.includeRiskAllowance) {
        earningsRows.push([`RISK ALLOWANCE (${formatPercent(entry.riskRate)})`, entry.risk]);
      }
      if (entry.includeLeaveAllowance) {
        earningsRows.push([`LEAVE ALLOWANCE (${formatPercent(entry.leaveAllowanceRate)})`, entry.leaveAllowance]);
      }
      if (entry.includeClothingAllowance) {
        earningsRows.push([`CLOTHING ALLOWANCE (${formatPercent(entry.clothingRate)})`, entry.clothingAllowance]);
      }
      earningsRows.push(
      ...
      const filteredDeductionRows: Array<[string, number]> = deductionRows.filter(
        (row): row is [string, number] => !isZeroAmount(row[1])
      );

      const standardRowCount = Math.max(filteredEarningsRows.length, filteredDeductionRows.length);
      ...
      autoTable(doc, {
        ...
        didParseCell: (data) => {
          ...
          const isSummaryRow =
            data.row.index === grossSummaryRowIndex || data.row.index === taxableSummaryRowIndex;
          ...
          const isNetSalaryCell =
            data.row.index === taxableSummaryRowIndex && (data.column.index === 2 || data.column.index === 3);
    ...
    if (includeAllStaffSummary) {
      ...
      autoTable(doc, {
        ...
        margin: { left: margin, right: margin },
        tableWidth: pageInnerWidth,
        head: [["EMPLOYEE", "GROSS", "TAXABLE", "PAYE", "DEDUCTIONS", "NET", "OUTSTANDING"]],
        body: sourceEntries.map((entry) => [
          entry.name,
          isZeroAmount(entry.grossEarnings) ? "" : formatPdfMoney(entry.grossEarnings),
          isZeroAmount(entry.taxableIncome) ? "" : formatPdfMoney(entry.taxableIncome),
          isZeroAmount(entry.paye) ? "" : formatPdfMoney(entry.paye),
          isZeroAmount(entry.totalDeductions) ? "" : formatPdfMoney(entry.totalDeductions),
          isZeroAmount(entry.netSalary) ? "" : formatPdfMoney(entry.netSalary),
          isZeroAmount(entry.outstanding) ? "" : formatPdfMoney(entry.outstanding)
        ]),
        foot: [
          [
            "TOTALS",
            isZeroAmount(totals.grossEarnings) ? "" : formatPdfMoney(totals.grossEarnings),
            isZeroAmount(totals.taxableIncome) ? "" : formatPdfMoney(totals.taxableIncome),
            isZeroAmount(totals.paye) ? "" : formatPdfMoney(totals.paye),
            isZeroAmount(totals.totalDeductions) ? "" : formatPdfMoney(totals.totalDeductions),
            isZeroAmount(totals.netSalary) ? "" : formatPdfMoney(totals.netSalary),
            isZeroAmount(totals.outstanding) ? "" : formatPdfMoney(totals.outstanding)
          ]
        ],
        styles: {
          font: "helvetica",
  ...
  const handleDownloadEntrySlip = async (entry: PayslipEntry) => {
    const safeName = entry.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    const date = new Date();
    const y = date.getFullYear();
    const m = String(date.getMonth() + 1).padStart(2, "0");
    const d = String(date.getDate()).padStart(2, "0");
  ...
  const handleExport = async () => {
    ...
    await exportPdf(report.title, report.headers, sanitizedRows, report.filename);
  };
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-8">
        <header className="flex flex-col gap-3">
          ...
          <p className="max-w-4xl text-sm text-[color:var(--text-secondary)] md:text-base">
            Payroll aligned to your slip: provident earning lines (3.5% and 6.5%) are included in gross earnings,
            taxable income is calculated as gross minus Social Security (5.5%) and Provident Deduction (16.5%), then
            PAYE is applied.
          </p>
        </header>
          ...
          <div className="rounded-2xl border border-[color:var(--card-border)] bg-[color:var(--card-bg)] p-6 shadow-card backdrop-blur">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="text-xl font-semibold text-[color:var(--text-primary)]">Employee Payroll Details</h2>
                {editingEntryId ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={clearEditor}
                  className="inline-flex items-center justify-center rounded-full border border-[color:var(--border-default)] px-4 py-2 text-sm font-semibold text-[color:var(--text-secondary)]"
                >
                  Reset Calculations
                </button>
              </div>
            ...
            <p className="mt-4 text-xs text-[color:var(--text-secondary)]">
              Values auto-calculate as you type. Comma-formatted amounts like `22,000` are supported.
            </p>

            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <div className="md:col-span-2">
                <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">Employee Name</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">Basic Salary (GHS)</label>
                <input
                  type="text"
                  inputMode="decimal"
                  value={form.basic}
              ...
              <div>
                ...
                <div className="mt-2 grid grid-cols-[1fr_1fr] gap-2">
                  <input
                    ...
                    onChange={(e) => updateField("transportRate", e.target.value)}
                    placeholder="25"
                  ...
                  <input
                    type="text"
                    inputMode="decimal"
                    value={allowanceOverrides.transport ? form.transport : toInputValue(calculations.transport)}
                    onChange={(e) => updateAllowanceAmount("transport", "transport", e.target.value)}
                    placeholder="0.00"
                ...
                <label className="mt-2 inline-flex items-center gap-2 text-xs text-[color:var(--text-secondary)]">
                  <input
                    ...
                    onChange={(e) => {
                      const checked = e.target.checked;
                      if (checked) updateField("transport", toInputValue(calculations.transport));
              ...
              <div>
                ...
                <div className="mt-2 grid grid-cols-[1fr_1fr] gap-2">
                  ...
                  <input
                    type="text"
                    inputMode="decimal"
                    value={allowanceOverrides.utility ? form.utility : toInputValue(calculations.utility)}
                    onChange={(e) => updateAllowanceAmount("utility", "utility", e.target.value)}
                    placeholder="0.00"
                ...
                <label className="mt-2 inline-flex items-center gap-2 text-xs text-[color:var(--text-secondary)]">
                  <input
                    ...
                    onChange={(e) => {
                      const checked = e.target.checked;
                      if (checked) updateField("utility", toInputValue(calculations.utility));
              ...
              <div>
                ...
                <div className="mt-2 grid grid-cols-[1fr_1fr] gap-2">
                  ...
                  <input
                    type="text"
                    inputMode="decimal"
                    value={allowanceOverrides.rent ? form.rent : toInputValue(calculations.rent)}
                    onChange={(e) => updateAllowanceAmount("rent", "rent", e.target.value)}
                    placeholder="0.00"
                ...
                <label className="mt-2 inline-flex items-center gap-2 text-xs text-[color:var(--text-secondary)]">
                  <input
                    ...
                    onChange={(e) => {
                      const checked = e.target.checked;
                      if (checked) updateField("rent", toInputValue(calculations.rent));
              ...
              <div>
                ...
                <div className="mt-2 grid grid-cols-[1fr_1fr] gap-2">
                  ...
                  <input
                    type="text"
                    inputMode="decimal"
                    value={allowanceOverrides.risk ? form.risk : toInputValue(calculations.risk)}
                    onChange={(e) => updateAllowanceAmount("risk", "risk", e.target.value)}
                    placeholder="0.00"
                ...
                <label className="mt-2 inline-flex items-center gap-2 text-xs text-[color:var(--text-secondary)]">
                  <input
                    ...
                    onChange={(e) => {
                      const checked = e.target.checked;
                      if (checked) updateField("risk", toInputValue(calculations.risk));
              ...
              <div className="md:col-span-2 rounded-xl border border-[color:var(--border-subtle)] p-3">
                <label className="inline-flex items-center gap-2 text-sm font-semibold text-[color:var(--text-tertiary)]">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!checked) updateAllowanceOverride("leaveAllowance", false);
                    }}
                  />
                ...
                <div className="mt-3 grid grid-cols-[1fr_1fr] gap-2">
                  <input
                    ...
                    onChange={(e) => updateField("leaveAllowanceRate", e.target.value)}
                    placeholder="100"
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateAllowanceAmount("leaveAllowance", "leaveAllowance", e.target.value)
                    }
                    placeholder="0.00"
                ...
                <label className="mt-2 inline-flex items-center gap-2 text-xs text-[color:var(--text-secondary)]">
                  <input
                    ...
                    onChange={(e) => {
                      const checked = e.target.checked;
                      if (checked) updateField("leaveAllowance", toInputValue(calculations.leaveAllowance));
              ...
              <div>
                ...
                <label className="mt-2 inline-flex items-center gap-2 text-xs text-[color:var(--text-secondary)]">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!checked) updateAllowanceOverride("clothingAllowance", false);
                    }}
                  />
                  Include clothing
                </label>
                <div className="mt-2 grid grid-cols-[1fr_1fr] gap-2">
                  <input
                    ...
                    onChange={(e) => updateField("clothingRate", e.target.value)}
                    placeholder="0"
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateAllowanceAmount("clothingAllowance", "clothingAllowance", e.target.value)
                    }
                    placeholder="0.00"
                ...
                <label className="mt-2 inline-flex items-center gap-2 text-xs text-[color:var(--text-secondary)]">
                  <input
                    ...
                    onChange={(e) => {
                      const checked = e.target.checked;
                      if (checked) {
                        updateField("clothingAllowance", toInputValue(calculations.clothingAllowance));
                      }
              ...
              <div>
                <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">Other Earnings</label>
                <input
                  type="text"
                  inputMode="decimal"
              ...
              <div>
                <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">Overtime Addition</label>
                <input
                  type="text"
                  inputMode="decimal"
                  value={form.overtimeAddition}
                  onChange={(e) => updateField("overtimeAddition", e.target.value)}
                  placeholder="0.00"
                  className="mt-2 w-full rounded-xl border border-[color:var(--border-default)] bg-[color:var(--surface)] px-3 py-2 text-sm text-[color:var(--text-tertiary)] shadow-sm focus:border-[color:var(--accent)] focus:outline-none"
                />
              </div>
              ...
              <div>
                <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">Welfare Fund</label>
                <input
                  type="text"
                  inputMode="decimal"
                  value={form.welfare}
              ...
              <div>
                <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">Union Dues</label>
                <input
                  type="text"
                  inputMode="decimal"
              ...
              <div>
                <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">PMSU/GMWU Support</label>
                <input
                  type="text"
                  inputMode="decimal"
                  value={form.support}
              ...
              <div>
                <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">Lateness Deduction</label>
                <input
                  type="text"
                  inputMode="decimal"
                  value={form.latenessDeduction}
                  onChange={(e) => updateField("latenessDeduction", e.target.value)}
                  placeholder="0.00"
                  className="mt-2 w-full rounded-xl border border-[color:var(--border-default)] bg-[color:var(--surface)] px-3 py-2 text-sm text-[color:var(--text-tertiary)] shadow-sm focus:border-[color:var(--accent)] focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateField("absenteeismDeduction", e.target.value)}
                  placeholder="0.00"
                  className="mt-2 w-full rounded-xl border border-[color:var(--border-default)] bg-[color:var(--surface)] px-3 py-2 text-sm text-[color:var(--text-tertiary)] shadow-sm focus:border-[color:var(--accent)] focus:outline-none"
                />
              </div>
              ...
              <div>
                <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">Loan Amount</label>
                <input
                  type="text"
                  inputMode="decimal"
              ...
              <div>
                <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">Total Paid (Loan)</label>
                <input
                  type="text"
                  inputMode="decimal"
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="flex flex-col gap-2">
                <p className="text-xs font-semibold tracking-[0.08em] text-[color:var(--accent)]">Summary</p>
                <h2 className="text-xl font-semibold text-[color:var(--text-primary)]">Auto-calculated</h2>
                <p className="text-sm text-[color:var(--text-secondary)]">
                  Taxable = Gross - Social Security (5.5%) - Provident Deduction (16.5%).
                </p>
              </div>
            ...
            <div className="mt-6 grid gap-3 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-[color:var(--text-primary)]">
                  {calculations.includeTransportAllowance ? formatMoney(calculations.transport) : "Off"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">
                ...
                <span className="font-semibold text-[color:var(--text-primary)]">
                  {calculations.includeUtilityAllowance ? formatMoney(calculations.utility) : "Off"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">
                ...
                <span className="font-semibold text-[color:var(--text-primary)]">
                  {calculations.includeRentAllowance ? formatMoney(calculations.rent) : "Off"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">
                ...
                <span className="font-semibold text-[color:var(--text-primary)]">
                  {calculations.includeRiskAllowance ? formatMoney(calculations.risk) : "Off"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">
                ...
                <span className="font-semibold text-[color:var(--text-primary)]">
                  {calculations.includeLeaveAllowance ? formatMoney(calculations.leaveAllowance) : "Off"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">Prov. earning 3.5%</span>
                <span className="font-semibold text-[color:var(--text-primary)]">{formatMoney(calculations.providentEarning35)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">Prov. earning 6.5%</span>
                <span className="font-semibold text-[color:var(--text-primary)]">{formatMoney(calculations.providentEarning65)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">Overtime addition</span>
                <span className="font-semibold text-[color:var(--text-primary)]">
                  {formatMoney(calculations.overtimeAddition)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">Gross earnings</span>
                <span className="font-semibold text-[color:var(--text-primary)]">{formatMoney(calculations.grossEarnings)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">Social Security (5.5%)</span>
                <span className="font-semibold text-[color:var(--text-primary)]">{formatMoney(calculations.socialSecurity)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">Provident Deduction (16.5%)</span>
                <span className="font-semibold text-[color:var(--text-primary)]">{formatMoney(calculations.providentDeduction165)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">Taxable income</span>
                <span className="font-semibold text-[color:var(--text-primary)]">{formatMoney(calculations.taxableIncome)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">PAYE</span>
                <span className="font-semibold text-[color:var(--text-primary)]">{formatMoney(calculations.paye)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">Lateness deduction</span>
                <span className="font-semibold text-[color:var(--text-primary)]">
                  {formatMoney(calculations.latenessDeduction)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[color:var(--text-secondary)]">Absenteeism deduction</span>
                <span className="font-semibold text-[color:var(--text-primary)]">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-[color:var(--text-primary)]">{formatMoney(calculations.totalDeductions)}</span>
              </div>
              <div className="flex items-center justify-between text-base">
                <span className="font-semibold text-[color:var(--accent-strong)]">Net salary</span>
                <span className="text-lg font-semibold text-[color:var(--accent-strong)]">{formatMoney(calculations.netSalary)}</span>
              </div>
              <div className="flex items-center justify-between text-xs text-[color:var(--text-secondary)]">
                <span>Outstanding loan</span>
                <span className="font-semibold text-[color:var(--text-primary)]">{formatMoney(calculations.outstanding)}</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-[color:var(--card-border)] bg-[color:var(--card-bg)] p-6 shadow-card backdrop-blur">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-semibold tracking-[0.08em] text-[color:var(--accent)]">Employees</p>
              <h2 className="text-xl font-semibold text-[color:var(--text-primary)]">GB Payroll Table</h2>
            </div>
            <p className="text-sm text-[color:var(--text-secondary)]">
          ...
          <div className="mt-5 overflow-x-auto">
            <table className="min-w-[2300px] w-full border-collapse text-sm">
              ...
              <tbody>
                {entries.length === 0 ? (
                  <tr>
                    <td colSpan={29} className="py-6 text-center text-sm text-[color:var(--text-secondary)]">
                  ...
                  entries.map((entry) => (
                    <tr key={entry.id} className="border-b border-[color:var(--border-subtle)]">
                      <td className="py-3 pr-4 font-semibold text-[color:var(--text-primary)]">{entry.name}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.basic)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.transport)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.utility)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.rent)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.risk)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.leaveAllowance)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.clothingAllowance)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.otherEarnings)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.overtimeAddition)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.providentEarning35)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.providentEarning65)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.grossEarnings)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.socialSecurity)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.providentDeduction35)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.providentDeduction165)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.taxableIncome)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.paye)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.welfare)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.unionDues)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.support)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.latenessDeduction)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.absenteeismDeduction)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.totalDeductions)}</td>
                      <td className="py-3 pr-4 font-semibold text-[color:var(--accent-strong)]">{formatMoney(entry.netSalary)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.loanAmount)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.totalPaid)}</td>
                      <td className="py-3 pr-4">{formatMoney(entry.outstanding)}</td>
                      <td className="py-3 text-right">
                        <div className="flex justify-end gap-2">
                          <button
                            type="button"
                            onClick={() => handleViewEntry(entry)}
                            className="rounded-full border border-[color:var(--border-default)] px-3 py-1 text-xs font-semibold text-[color:var(--text-secondary)]"
                          >
                            View
                          </button>
                          <button
        ...
        <section className="rounded-2xl border border-[color:var(--card-border)] bg-[color:var(--card-bg)] p-6 shadow-card backdrop-blur">
          <div className="flex flex-col gap-2">
            <p className="text-xs font-semibold tracking-[0.08em] text-[color:var(--accent)]">Reports</p>
              <h2 className="text-xl font-semibold text-[color:var(--text-primary)]">Export Payroll and Deductions</h2>
            <p className="text-sm text-[color:var(--text-secondary)]">
              Generate PDF, CSV, or JSON files using the GB calculator, including a simple printer-friendly payslip PDF.
            </p>
          </div>

          <div className="mt-6 grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end">
            <div>
              <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">Report Type</label>
              <select
                value={reportType}
            ...
            <div>
              <label className="text-sm font-semibold text-[color:var(--text-tertiary)]">File Format</label>
              <select
                value={reportFormat}
          ...
          <div className="mt-6 rounded-xl border border-[color:var(--border-default)] bg-[color:var(--surface)] p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-sm font-semibold text-[color:var(--text-primary)]">{previewReport.title} Preview</p>
            ...
            {entries.length === 0 ? (
              <p className="mt-3 text-sm text-[color:var(--text-secondary)]">Add employees to see the report preview.</p>
            ) : (
      ...
      {viewingEntry ? (
        ...
        >
          <div
            role="dialog"
            aria-modal="true"
            className="max-h-[88vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-[color:var(--card-border)] bg-[color:var(--card-bg)] p-6 shadow-card"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold tracking-[0.08em] text-[color:var(--accent)]">View</p>
                <h3 className="text-xl font-semibold text-[color:var(--text-primary)]">{viewingEntry.name}</h3>
              </div>
              <button
                type="button"
                onClick={() => setViewingEntry(null)}
                className="rounded-full border border-[color:var(--border-default)] px-4 py-2 text-sm font-semibold text-[color:var(--text-secondary)]"
              >
                Close
              </button>
            </div>
            ...
            <div className="mt-6 grid gap-6 md:grid-cols-2">
              <div className="rounded-xl border border-[color:var(--border-default)] p-4">
                <h4 className="text-sm font-semibold text-[color:var(--text-primary)]">Earnings</h4>
                <div className="mt-3 grid gap-2 text-sm">
                  <div className="flex items-center justify-between">
                    <span>Basic</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.basic)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Transport</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span>Utility</span>
                    <span className="font-semibold">
                      {viewingEntry.includeUtilityAllowance ? formatMoney(viewingEntry.utility) : "Off"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Rent</span>
                    <span className="font-semibold">
                      {viewingEntry.includeRentAllowance ? formatMoney(viewingEntry.rent) : "Off"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Risk</span>
                    <span className="font-semibold">
                      {viewingEntry.includeRiskAllowance ? formatMoney(viewingEntry.risk) : "Off"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Leave</span>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold">{formatMoney(viewingEntry.otherEarnings)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Overtime</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.overtimeAddition)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Prov. 3.5%</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.providentEarning35)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Prov. 6.5%</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.providentEarning65)}</span>
                  </div>
                  <div className="flex items-center justify-between border-t border-[color:var(--border-subtle)] pt-2">
                    ...
                    <span className="font-semibold">{formatMoney(viewingEntry.grossEarnings)}</span>
                  </div>
                </div>
              </div>

              <div className="rounded-xl border border-[color:var(--border-default)] p-4">
                <h4 className="text-sm font-semibold text-[color:var(--text-primary)]">Deductions</h4>
                <div className="mt-3 grid gap-2 text-sm">
                  <div className="flex items-center justify-between">
                    <span>Social Security (5.5%)</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.socialSecurity)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>PAYE</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.paye)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Welfare</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.welfare)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Union dues</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.unionDues)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Support</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.support)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Lateness</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.latenessDeduction)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Absenteeism</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.absenteeismDeduction)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Provident 3.5%</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.providentDeduction35)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Provident 16.5%</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.providentDeduction165)}</span>
                  </div>
                  <div className="flex items-center justify-between border-t border-[color:var(--border-subtle)] pt-2">
                    <span className="font-semibold">Total deductions</span>
                    <span className="font-semibold">{formatMoney(viewingEntry.totalDeductions)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 rounded-xl border border-[color:var(--border-default)] p-4">
              <h4 className="text-sm font-semibold text-[color:var(--text-primary)]">Summary</h4>
              <div className="mt-3 grid gap-2 text-sm md:grid-cols-2">
                <div className="flex items-center justify-between">
                  <span>Taxable income</span>
                  <span className="font-semibold">{formatMoney(viewingEntry.taxableIncome)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Net salary</span>
                  <span className="font-semibold">{formatMoney(viewingEntry.netSalary)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Loan amount</span>
                  <span className="font-semibold">{formatMoney(viewingEntry.loanAmount)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Total paid</span>
                  <span className="font-semibold">{formatMoney(viewingEntry.totalPaid)}</span>
                </div>
                <div className="flex items-center justify-between md:col-span-2">
                  <span className="font-semibold">Outstanding loan</span>
                  <span className="font-semibold">{formatMoney(viewingEntry.outstanding)}</span>
                </div>
              </div>
            </div>
          </div>
